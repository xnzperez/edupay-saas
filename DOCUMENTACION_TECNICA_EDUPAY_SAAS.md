...
*   **Control de Acceso y Enrutamiento Férreo (RBAC Guards):** La topología de navegación protege las rutas mediante componentes de orden superior (HOCs). Implementaciones como `MasterRouteGuard`, `PrivateRoute` y `RoleRoute` envuelven las vistas encapsuladas. Esto garantiza que un estudiante jamás pueda montar en el DOM (Virtual DOM) un componente administrativo; la validación basada en JWT y roles desde `Zustand` intercepta la navegación y eyecta al usuario no autorizado en tiempo de reconciliación antes de que el componente sea interpretado por el motor JSX.
...
*   **Capa 3: Control de Acceso Transversal (RBAC Engine):** En la espina dorsal del backend, `auth/rbac.go` orquesta un Middleware implacable de Roles y Privilegios. No basta con inyectar un JWT válido; la ejecución de *Handlers* es inspeccionada algorítmicamente en tiempo `O(1)` frente a una matriz perimetral de roles (`Superadmin`, `Cajero`, `Student`). Toda mutación intrusiva de un rol menor hacia recursos mayores levanta una excepción `403 Forbidden` irrecuperable.
*   **Paginación Determinista y Sanitizada:** Ante la exposición a tablas transaccionales masivas, se ha erradicado el riesgo de ataques *DDoS por Agotamiento de Tiempos de Búsqueda*. El módulo de utilidades base (`utils/pagination.go`) fuerza constantes matemáticas de partición de registros (Limit/Offset). La aplicación inhibe intentos de extraer *dumps* masivos por sobre la capacidad de hardware.
...
### 3.5 Ecosistema Transaccional Extendido y Módulos de Valor Agregado
A medida que la lógica empresarial ha escalado, el sistema absorbió dominios operativos adicionales para consolidar a EduPay como una plataforma transaccional 360° en la institución. Se introdujeron las siguientes arquitecturas críticas:

*   **Módulo de Tienda de Circuito Cerrado (E-Commerce Interno):** Más allá del procesamiento rígido de mensualidades, EduPay integra un motor modular de tienda (`store`) donde las instituciones ofertan inventarios y servicios (uniformes, constancias, certificaciones). El débito se efectúa transaccionalmente contra el saldo de la billetera digital y cuenta con trazabilidad hiperestricta (`db.Beginx()`). En la misma transacción bloqueante se corrobora disponibilidad del ítem, se descuenta el fondo del *Wallet* y se impacta el historial de órdenes, previniendo en todo momento un sobregiro o déficit fantasma de stock.
*   **Micropagos P2P y Grafo de Beneficiarios (Contacts):** La plataforma ha incrustado un circuito de transferencias financieras asíncronas de igual a igual (Peer-to-Peer). Acoplado a esto, existe un gestor de *Contacts* que valida preventivamente unívocamente al compañero estudiantil bajo un filtro severo de *Cross-Tenant Isolation* (ambos deben residir en la base de la misma institución matriz). Para erradicar los clásicos abrazos mortales matemáticos (*Distributed Deadlocks*) en la alteración concurrente de dos saldos de billeteras cruzadas, la Base de Datos aplica jerarquía y bloqueo secuencial por ordenamiento léxico de los UUID de las carteras originarias e implicadas.
*   **Generación Vectorial e In-Memory de PDFs (Recibos):** El ecosistema prohíbe explícitamente el uso de APIs gratuitas de reportería en la nube que sabotean la confidencialidad de la matrícula, introducen retrasos por TTI y venden metadatos. En su lugar, EduPay integra y provee tuberías nativas internas interactivas en Go (`billing/pdf.go` y `store/pdf.go`) que compilan reportes y facturas corporativas cien por ciento en RAM. La representación gráfica se devuelve destilada hacia la API por el buffer HTTP bajo MIME Type `application/pdf`, logrando entregas instantáneas sub-milisegundo absolutas.
*   **Tubería de Notificaciones Desacoplada (Mailer Asíncrono):** Durante movimientos dinerarios críticos (recargas o transferencias magnas), el módulo `mailer.go` es detonado. Su invocación está arquitectónicamente deslindada del contexto HTTP finalizante de Fiber, operando sobre una capa apartada de procesamiento transaccional (Goroutines estructuradas tipo *Fire-And-Forget*). Con esto, la aplicación concede un estatus resuelto de `200 OK` raudo al navegador del cliente en paralelo, cediendo milisegundos invaluables para que el servidor de correo emisor pueda negociar la inyección SMTP sobre la bandeja de entrada del receptor.
